...
class MusicLibraryController
  ...
  def list_songs
    ...
    alpha_song.each.with_index(1) do |song,index|
      puts "#{index}. #{song.name} -#{song.artist.name}  - #{song.genre.name}"
    end 
  end

  def list_artists
    alpha_artist = Artist.all.sort_by do |artist| 
      artist.name
    end
    alpha_artist.each.with_index(1) do |artist,index|
      puts "#{index}. #{artist.name}"
    end
  end

  def list_genres
    alpha_genre = Genre.all.sort_by do |genre|
      genre.name
    end
    alpha_genre.each.with_index(1) do |genre,index|
